...
const DestinationMenu:React.FC<Props> = ({ activePlanet, hoveredPlanet, handleClick, handleMouseEnter, handleMouseLeave}) => {
  ...
  return (
    <ul className='flex justify-between w-60 mb-5 tablet:w-72 tablet:mb-8 desktop:mb-9'>
    {
      destinationData.map(planet =>    
        <li 
          key={planet.name} 
          className={` relative h-7 cursor-pointer tablet:h-8`} 
        ...
        >
            ...
            <div 
              className={`indicator absolute left-0 bottom-0 w-full h-[0.1875rem] bg-white 
              ${activePlanet === planet.name ? 'opacity-100' : hoveredPlanet === planet.name ? 'opacity-50' : 'opacity-0'}`}>
            </div> 
        </li>  
      )
    }
